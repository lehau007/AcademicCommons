"""Groq provider (final fallback). Ports experiment ``_call_groq``.

Vision works only when the configured Groq model is vision-capable; otherwise used
for text normalization fallback.
"""
...
class GroqVisionProvider(VisionLanguageProvider):
    provider_name = "groq"
    ...
    def complete(
        ...
        *,
        ...
        if not HAS_OPENAI:
            ...
            self._recorder.record(operation, "groq", model, "error", 0, error=msg)
        ...
        try:
            ...
            self._recorder.record(operation, "groq", model, "error", latency_ms, error=err_detail)
            self._emitter.emit(
                "llm_call_error",
                operation=operation,
                provider="groq",
