<!-- slide 1 -->
![figure: title/cover slide image (no selectable text layer)]
...
```
*(Chú thích: "Khai báo tên của cấu trúc"; "Các trường của cấu trúc"; "Đừng quên dấu ; sau khai báo cấu trúc")*
...
```
*(Chú thích: "Tên kiểu cấu trúc"; "Tên của biến")*
...
```
*(Chú thích: "Khởi tạo các trường cấu trúc giống như đối với các biến thông thường")*
...
```
*(Chú thích: "Khai báo một con trỏ có kiểu cấu trúc")*
...
```
*(Chú thích: "Cấu trúc dữ liệu hiện có"; "Tên kiểu mới"; "Khai báo biến, con trỏ hay mảng với kiểu dữ liệu mới")*
...
Student stud_list[100];
```
*(Chú thích: "Gộp định nghĩa kiểu với khai báo cấu trúc")*
...
![figure: a large red ✗ (cross) icon is placed to the right of the `if(studA == studB){...}` code block, indicating that comparing two structs with the `==` operator is **wrong**.]
...
![figure: a large green ✓ (check mark) icon is placed to the right of the field-by-field `strcmp`/`&&` code block, indicating that comparing structs **field by field** is the correct approach.]

<!-- slide 10 -->
# MẢNG CẤU TRÚC

![figure: diagram of an array `stud_list` with indices 0..3, each element a struct { name, mark }: [0] name:"fred", mark:5; [1] name:"ralph", mark:7; [2] name:"fong", mark:4; [3] name:"rachel", mark:9. Label "stud_list[0]" points to the first element.]
